...
class Trainer:
    def __init__(self, cfg):
        ...
        self.best_metric_value = torch.tensor([-1.0e100])
        # TimeEstimator
    ...
    def after_epoch(self):
        ...
        # self.logger.info('Train result-row: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(
        #     m_iou, m_acc, all_acc))
        self.logger.info('Train result-row: loss {:.4f}.'.format(loss_avg))
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.val_loader):
            ...
            for key in input_dict.keys():
                input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                output = self.model(input_dict, use_prototype=False)
        ...
        self.logger.info('Val result-row: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(
            m_iou, m_acc, all_acc))
