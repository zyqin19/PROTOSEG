...
class SegmentationTest(object):
    ...
    def __call__(self, cfg, test_loader, model):
        ...
        for idx in range(len(test_dataset)):
            ...
            if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
                ...
                for i in range(batch_num):
                    ...
                    with torch.no_grad():
                        pred_part = model(input_dict, use_prototype=False)  # (n, k)
        ...
        logger.info('Val result-row: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}'.format(mIoU, mAcc, allAcc))
...
#         logger.info('Val result-row: ins.mIoU/cat.mIoU {:.4f}/{:.4f}.'.format(ins_mIoU, cat_mIoU))
...
class PartSegmentationTest(object):
    ...
    def __call__(self, cfg, test_loader, model):
        ...
        save_path = os.path.join(cfg.save_path, "result-row", "test_epoch{}".format(cfg.epochs))
        make_dirs(save_path)
